...
local filetype_settings = {go = {textwidth = 100, expandtab = false}, javascript = {expandtab = true, shiftwidth = 2}, javascriptreact = {expandtab = true, shiftwidth = 2}, typescript = {expandtab = true, shiftwidth = 2}, typescriptreact = {expandtab = true, shiftwidth = 2}, html = {expandtab = true, shiftwidth = 2}, css = {expandtab = true, shiftwidth = 2}, cs = {commentstring = "// %s"}, helm = {expandtab = true, shiftwidth = 2, commentstring = "{{/* %s */}}"}, gotmpl = {expandtab = true, shiftwidth = 2, commentstring = "{{/* %s */}}"}, fish = {expandtab = true, shiftwidth = 4, commentstring = "# %s"}, yaml = {expandtab = true, shiftwidth = 2}, ["buf-config"] = {expandtab = true, shiftwidth = 2}, svg = {expandtab = true, shiftwidth = 2}, json = {expandtab = true, shiftwidth = 2}, json5 = {expandtab = true, shiftwidth = 2}, bash = {expandtab = true, shiftwidth = 2}, toml = {expandtab = true, shiftwidth = 2}, python = {expandtab = true, shiftwidth = 4}, xml = {expandtab = true, shiftwidth = 4}, starlark = {expandtab = true, shiftwidth = 4, commentstring = "# %s"}, proto = {expandtab = true, shiftwidth = 2, commentstring = "// %s", cindent = true}, gitcommit = {spell = true, commentstring = "; %s"}, gitconfig = {shiftwidth = 2, expandtab = false}, fennel = {commentstring = ";; %s"}, sql = {wrap = true, commentstring = "-- %s", expandtab = true, shiftwidth = 4}, clojure = {expandtab = true, textwidth = 80}, kotlin = {commentstring = "// %s"}, just = {expandtab = true, shiftwidth = 4}, markdown = {spell = true, wrap = true, expandtab = false}}
do
...
vim.api.nvim_create_autocmd("BufNewFile", {pattern = "*", callback = _27_})
map("n", "<leader>du", vim.pack.update)
local function _29_()
  return vim.cmd({cmd = "Mason"})
end
map("n", "<leader>ma", _29_)
map("n", ";", ":")
local function _30_()
  return vim.cmd({cmd = "Git", mods = {vertical = true}})
end
map("n", "<leader>gs", _30_, {desc = "Open :Git in a vertical split"})
local function _31_()
  return vim.cmd({cmd = "Gwrite"})
end
map("n", "<leader>gw", _31_, {desc = ":Gwrite"})
local function _32_()
  return vim.cmd({cmd = "Git", args = {"commit"}})
end
map("n", "<leader>gc", _32_, {desc = ":Git commit"})
local function _33_()
  return vim.cmd({cmd = "Git", args = {"push"}})
end
map("n", "<leader>gp", _33_, {desc = ":Git push"})
local function _34_()
  return vim.cmd({cmd = "Git", args = {"blame"}})
end
map("n", "<leader>gb", _34_, {desc = ":Git blame"})
local function _35_()
...
for keymap, file in pairs({["<leader>ef"] = "$HOME/.config/fish/config.fish", ["<leader>egi"] = "$HOME/.config/git/config", ["<leader>ego"] = "$HOME/.config/ghostty/config", ["<leader>ek"] = "$HOME/.config/kitty/kitty.conf", ["<leader>ev"] = "$HOME/.config/nvim/init.fnl"}) do
  ...
  map("n", keymap, _39_, {desc = (":edit " .. file)})
end
local function _40_()
  return vim.cmd({cmd = "write"})
end
map("n", "<leader>w", _40_, {desc = ":write the buffer to the file"})
local function _41_()
  return vim.cmd({cmd = "split"})
end
map("n", "<leader>ss", _41_, {desc = "Create a horizontal split"})
local function _42_()
  return vim.cmd({cmd = "vsplit"})
end
map("n", "<leader>vs", _42_, {desc = "Create a vertical split"})
...
map("n", "<leader>tn", _44_, {desc = "Create a new tab"})
local function _45_()
  return vim.cmd({cmd = "tabnext"})
end
map("n", "]r", _45_, {desc = "Go to next tab"})
local function _46_()
  return vim.cmd({cmd = "tabprev"})
end
map("n", "[r", _46_, {desc = "Go to prev tab"})
...
local function lsp_attach(_47_)
  local buf = _47_.buf
  local _arg_48_ = _47_.data
