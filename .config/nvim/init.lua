...
do
  ...
  for filetype, settings in pairs(filetype_settings) do
    ...
    vim.api.nvim_create_autocmd("FileType", {group = aufiletypes, pattern = filetype, callback = _16_})
  end
end
...
map("n", "<leader>gs", _19_, {desc = "Open :Git in a vertical split"})
local function _20_()
  return vim.cmd({cmd = "Gwrite"})
end
map("n", "<leader>gw", _20_, {desc = ":Gwrite"})
local function _21_()
  return vim.cmd({cmd = "Git", args = {"commit"}})
end
map("n", "<leader>gc", _21_, {desc = ":Git commit"})
local function _22_()
  return vim.cmd({cmd = "Git", args = {"push"}})
end
map("n", "<leader>gp", _22_, {desc = ":Git push"})
local function _23_()
  return vim.cmd({cmd = "Git", args = {"blame"}})
end
map("n", "<leader>gb", _23_, {desc = ":Git blame"})
local function _24_()
...
for keymap, file in pairs({["<leader>ef"] = "$HOME/.config/fish/config.fish", ["<leader>egi"] = "$HOME/.config/git/config", ["<leader>ego"] = "$HOME/.config/ghostty/config", ["<leader>ek"] = "$HOME/.config/kitty/kitty.conf", ["<leader>ev"] = "$HOME/.config/nvim/init.fnl"}) do
  ...
  map("n", keymap, _28_, {desc = (":edit " .. file)})
end
local function _29_()
  return vim.cmd({cmd = "write"})
end
map("n", "<leader>w", _29_, {desc = ":write the buffer to the file"})
local function _30_()
  return vim.cmd({cmd = "close"})
end
map("n", "<leader>cl", _30_, {desc = ":close the current window"})
local function _31_()
  return vim.cmd({cmd = "split"})
end
map("n", "<leader>ss", _31_, {desc = "Create a horizontal split"})
local function _32_()
  return vim.cmd({cmd = "vsplit"})
end
map("n", "<leader>vs", _32_, {desc = "Create a vertical split"})
...
map("n", "<leader>tn", _33_, {desc = "Create a new tab"})
local function _34_()
  return vim.cmd({cmd = "tabnext"})
end
map("n", "]r", _34_, {desc = "Go to next tab"})
local function _35_()
  return vim.cmd({cmd = "tabprev"})
end
map("n", "[r", _35_, {desc = "Go to prev tab"})
...
local function lsp_attach(_36_)
  local buf = _36_["buf"]
  local _arg_37_ = _36_["data"]
  local client_id = _arg_37_["client_id"]
