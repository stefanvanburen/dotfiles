...
do
  ...
  vim.api.nvim_create_autocmd("BufNewFile", {group = vim.api.nvim_create_augroup("templates", {}), pattern = "*", callback = _35_})
end
...
map("n", "<leader>ch", _39_, {desc = ":checkhealth"})
...
map("n", "<leader>gP", _45_, {desc = ":PR"})
local function _46_()
  return vim.cmd({cmd = "Issue"})
end
map("n", "<leader>gi", _46_, {desc = ":Issue"})
local function _47_()
  return vim.cmd({cmd = "CI"})
end
map("n", "<leader>gA", _47_, {desc = ":CI"})
local function _48_()
...
map({"n", "v"}, "j", _48_, {expr = true, desc = "Down by visual line (gj when no count)"})
local function _50_()
  if (vim.v.count ~= 0) then
    return "k"
  else
...
map({"n", "v"}, "k", _50_, {expr = true, desc = "Up by visual line (gk when no count)"})
...
for keymap, file in pairs({["<leader>ef"] = "$HOME/.config/fish/config.fish", ["<leader>egi"] = "$HOME/.config/git/config", ["<leader>ego"] = "$HOME/.config/ghostty/config", ["<leader>ev"] = "$HOME/.config/nvim/init.fnl"}) do
  ...
  map("n", keymap, _52_, {desc = (":edit " .. file)})
end
local function _53_()
  return vim.cmd({cmd = "write"})
end
map("n", "<leader>w", _53_, {desc = ":write the buffer to the file"})
local function _54_()
  return vim.cmd({cmd = "split"})
end
map("n", "<leader>ss", _54_, {desc = "Create a horizontal split"})
local function _55_()
  return vim.cmd({cmd = "vsplit"})
end
map("n", "<leader>vs", _55_, {desc = "Create a vertical split"})
...
map("n", "<leader>i", _56_, {desc = "Inspect position (treesitter/syntax)"})
...
map("n", "<leader>tn", _57_, {desc = "Create a new tab"})
local function _58_()
  return vim.cmd({cmd = "tabnext"})
end
map("n", "]r", _58_, {desc = "Go to next tab"})
local function _59_()
  return vim.cmd({cmd = "tabprev"})
end
map("n", "[r", _59_, {desc = "Go to prev tab"})
...
local function lsp_attach(_60_)
  local buf = _60_.buf
  local _arg_61_ = _60_.data
  local client_id = _arg_61_.client_id
  ...
  if client:supports_method("textDocument/codeAction") then
    ...
    map("n", "gro", _62_, {desc = "Organize Imports"})
  else
  end
...
local function venv_cmd(name)
  local function _69_(dispatchers, config)
