...
do
  ...
  map("n", "-", _6_, {desc = "Open the file picker from the current file, or in the current working directory if the file does not exist"})
end
do
...
map("n", "<leader>db", _8_, {desc = "Open dadbod to the current $DATABASE_URL"})
...
map("n", "<leader>tt", _9_, {desc = "Run nearest test"})
local function _10_()
  return vim.cmd({cmd = "TestFile"})
end
map("n", "<leader>tf", _10_, {desc = "Run all tests in the file"})
do
...
do
  ...
  vim.api.nvim_create_autocmd("FileType", {pattern = treesitter_filetypes, callback = _16_})
end
do
...
local function seasonal_colorscheme()
  local now = os.date("*t")
  local case_17_ = now.month
  if (case_17_ == 1) then
    return "miniwinter"
  elseif (case_17_ == 2) then
    if (now.day < 4) then
      return "miniwinter"
    else
      return "minispring"
    end
  elseif (case_17_ == 3) then
    return "minispring"
  elseif (case_17_ == 4) then
    return "minispring"
  elseif (case_17_ == 5) then
    if (now.day < 6) then
      return "minispring"
    else
      return "minisummer"
    end
  elseif (case_17_ == 6) then
    return "minisummer"
  elseif (case_17_ == 7) then
    return "minisummer"
  elseif (case_17_ == 8) then
    if (now.day < 8) then
      return "minisummer"
    else
      return "miniautumn"
    end
  elseif (case_17_ == 9) then
    return "miniautumn"
  elseif (case_17_ == 10) then
    return "miniautumn"
  elseif (case_17_ == 11) then
    if (now.day < 8) then
      return "miniautumn"
    else
      return "miniwinter"
    end
  elseif (case_17_ == 12) then
    return "miniwinter"
  else
    return nil
  end
...
vim.api.nvim_create_autocmd("OptionSet", {pattern = "background", callback = _23_})
...
vim.api.nvim_create_autocmd("FileType", {group = vim.api.nvim_create_augroup("filetypes", {}), pattern = vim.tbl_keys(filetype_settings), callback = _30_})
local function _32_(_path, bufnr)
...
do
  ...
  vim.api.nvim_create_autocmd("BufNewFile", {pattern = "*", callback = _34_})
end
map("n", "<leader>du", vim.pack.update, {desc = "Update plugins"})
local function _37_()
  return vim.cmd({cmd = "Mason"})
end
map("n", "<leader>ma", _37_, {desc = ":Mason"})
map("n", ";", ":", {desc = "Enter command mode"})
local function _38_()
  return vim.cmd({cmd = "Git", mods = {vertical = true}})
end
map("n", "<leader>gs", _38_, {desc = "Open :Git in a vertical split"})
local function _39_()
  return vim.cmd({cmd = "Gwrite"})
end
map("n", "<leader>gw", _39_, {desc = ":Gwrite"})
local function _40_()
  return vim.cmd({cmd = "Git", args = {"commit"}})
end
map("n", "<leader>gc", _40_, {desc = ":Git commit"})
local function _41_()
  return vim.cmd({cmd = "Git", args = {"push"}})
end
map("n", "<leader>gp", _41_, {desc = ":Git push"})
local function _42_()
  return vim.cmd({cmd = "Git", args = {"blame"}})
end
map("n", "<leader>gb", _42_, {desc = ":Git blame"})
local function _43_()
...
map({"n", "v"}, "j", _43_, {expr = true, desc = "Down by visual line (gj when no count)"})
local function _45_()
  if (vim.v.count ~= 0) then
    return "k"
  else
...
map({"n", "v"}, "k", _45_, {expr = true, desc = "Up by visual line (gk when no count)"})
...
for keymap, file in pairs({["<leader>ef"] = "$HOME/.config/fish/config.fish", ["<leader>egi"] = "$HOME/.config/git/config", ["<leader>ego"] = "$HOME/.config/ghostty/config", ["<leader>ev"] = "$HOME/.config/nvim/init.fnl"}) do
  ...
  map("n", keymap, _47_, {desc = (":edit " .. file)})
end
local function _48_()
  return vim.cmd({cmd = "write"})
end
map("n", "<leader>w", _48_, {desc = ":write the buffer to the file"})
local function _49_()
  return vim.cmd({cmd = "split"})
end
map("n", "<leader>ss", _49_, {desc = "Create a horizontal split"})
local function _50_()
  return vim.cmd({cmd = "vsplit"})
end
map("n", "<leader>vs", _50_, {desc = "Create a vertical split"})
...
map("n", "<leader>i", _51_, {desc = "Inspect position (treesitter/syntax)"})
...
map("n", "<leader>tn", _52_, {desc = "Create a new tab"})
local function _53_()
  return vim.cmd({cmd = "tabnext"})
end
map("n", "]r", _53_, {desc = "Go to next tab"})
local function _54_()
  return vim.cmd({cmd = "tabprev"})
end
map("n", "[r", _54_, {desc = "Go to prev tab"})
...
local function lsp_attach(_55_)
  local buf = _55_.buf
  local _arg_56_ = _55_.data
  local client_id = _arg_56_.client_id
  ...
  if client:supports_method("textDocument/codeAction") then
    ...
    map("n", "gro", _57_, {desc = "Organize Imports"})
  else
  end
  ...
  if client:supports_method("textDocument/codeLens") then
    ...
    vim.api.nvim_create_autocmd({"BufEnter", "CursorHold", "InsertLeave"}, {group = augroup_id, buffer = buf, callback = _62_})
  else
  end
...
local function venv_cmd(name)
  local function _65_(dispatchers, config)
