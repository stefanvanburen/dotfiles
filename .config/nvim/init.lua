...
do
  ...
  map("n", "-", _8_, {desc = "Open the file picker from the current file, or in the current working directory if the file does not exist"})
end
do
...
map("n", "<leader>db", _10_, {desc = "Open dadbod to the current $DATABASE_URL"})
...
map("n", "<leader>tt", _11_, {desc = "Run nearest test"})
local function _12_()
...
do
  ...
  vim.api.nvim_create_autocmd("BufWritePost", {callback = _17_})
end
do
...
local filetype_settings = {go = {textwidth = 100, expandtab = false}, javascript = {expandtab = true, shiftwidth = 2}, javascriptreact = {expandtab = true, shiftwidth = 2}, typescript = {expandtab = true, shiftwidth = 2}, typescriptreact = {expandtab = true, shiftwidth = 2}, html = {expandtab = true, shiftwidth = 2}, css = {expandtab = true, shiftwidth = 2}, cs = {commentstring = "// %s"}, helm = {expandtab = true, shiftwidth = 2, commentstring = "{{/* %s */}}"}, gotmpl = {expandtab = true, shiftwidth = 2, commentstring = "{{/* %s */}}"}, fish = {expandtab = true, shiftwidth = 4, commentstring = "# %s"}, yaml = {expandtab = true, shiftwidth = 2}, svg = {expandtab = true, shiftwidth = 2}, json = {expandtab = true, shiftwidth = 2}, json5 = {expandtab = true, shiftwidth = 2}, bash = {expandtab = true, shiftwidth = 2}, toml = {expandtab = true, shiftwidth = 2}, python = {expandtab = true, shiftwidth = 4}, xml = {expandtab = true, shiftwidth = 4}, starlark = {expandtab = true, shiftwidth = 4, commentstring = "# %s"}, proto = {expandtab = true, shiftwidth = 2, commentstring = "// %s", cindent = true}, gitcommit = {spell = true}, gitconfig = {shiftwidth = 2, expandtab = false}, fennel = {commentstring = ";; %s"}, sql = {wrap = true, commentstring = "-- %s", expandtab = true, shiftwidth = 4}, clojure = {expandtab = true, textwidth = 80}, kotlin = {commentstring = "// %s"}, just = {expandtab = true, shiftwidth = 4}, markdown = {spell = true, wrap = true, expandtab = false}}
do
...
vim.api.nvim_create_autocmd("BufNewFile", {pattern = "*", callback = _21_})
map("n", "<leader>du", vim.pack.update)
local function _23_()
  return vim.cmd({cmd = "Mason"})
end
map("n", "<leader>ma", _23_)
map("n", ";", ":")
local function _24_()
  return vim.cmd({cmd = "Git", mods = {vertical = true}})
end
map("n", "<leader>gs", _24_, {desc = "Open :Git in a vertical split"})
local function _25_()
  return vim.cmd({cmd = "Gwrite"})
end
map("n", "<leader>gw", _25_, {desc = ":Gwrite"})
local function _26_()
  return vim.cmd({cmd = "Git", args = {"commit"}})
end
map("n", "<leader>gc", _26_, {desc = ":Git commit"})
local function _27_()
  return vim.cmd({cmd = "Git", args = {"push"}})
end
map("n", "<leader>gp", _27_, {desc = ":Git push"})
local function _28_()
  return vim.cmd({cmd = "Git", args = {"blame"}})
end
map("n", "<leader>gb", _28_, {desc = ":Git blame"})
local function _29_()
...
for keymap, file in pairs({["<leader>ef"] = "$HOME/.config/fish/config.fish", ["<leader>egi"] = "$HOME/.config/git/config", ["<leader>ego"] = "$HOME/.config/ghostty/config", ["<leader>ek"] = "$HOME/.config/kitty/kitty.conf", ["<leader>ev"] = "$HOME/.config/nvim/init.fnl"}) do
  ...
  map("n", keymap, _33_, {desc = (":edit " .. file)})
end
local function _34_()
  return vim.cmd({cmd = "write"})
end
map("n", "<leader>w", _34_, {desc = ":write the buffer to the file"})
local function _35_()
  return vim.cmd({cmd = "split"})
end
map("n", "<leader>ss", _35_, {desc = "Create a horizontal split"})
local function _36_()
  return vim.cmd({cmd = "vsplit"})
end
map("n", "<leader>vs", _36_, {desc = "Create a vertical split"})
...
map("n", "<leader>tn", _38_, {desc = "Create a new tab"})
local function _39_()
  return vim.cmd({cmd = "tabnext"})
end
map("n", "]r", _39_, {desc = "Go to next tab"})
local function _40_()
  return vim.cmd({cmd = "tabprev"})
end
map("n", "[r", _40_, {desc = "Go to prev tab"})
...
local function lsp_attach(_41_)
  local buf = _41_.buf
  local _arg_42_ = _41_.data
  local client_id = _arg_42_.client_id
  ...
  if client:supports_method("textDocument/codeAction") then
    ...
    map("n", "gro", _43_, {desc = "Organize Imports"})
  else
  end
